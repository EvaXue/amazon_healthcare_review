...
def get_product_reviews_url(item_id, page_number=None):
    if not page_number:
        page_number = 1
    tmp_url =  AMAZON_BASE_URL + '/product-reviews/{}/ref=' \
                             'cm_cr_arp_d_paging_btm_1?ie=UTF8&reviewerType=all_reviews' \
                             '&showViewpoints=1&sortBy=helpful&pageNumber={}'.format(
        item_id, page_number)
    print 'product url ---------'
    logging.info(tmp_url)
    logging.info('URL for product_id --- {}.'.format(item_id))
    return tmp_url
...
def get_comments_based_on_keyword(search):
    ...
    print 'comments on keywords url ---------'
    #print url
    logging.info('Product URL    = ' + url)
    soup = get_soup(url)
    # for div in soup.find_all('div'):
    #     if 'data-index' in div.attrs:
    #         print div.attrs.get('data-asin')

    product_ids = [div.attrs.get('data-asin') for div in soup.find_all('div') if 'data-index' in div.attrs]
...
def get_comments_with_product_id(product_id):
    ...
    #print so
    file = open("product_review.txt", "w")
    file.write(so.text.encode('ascii', 'ignore').decode('ascii'))
    file.close()
#####################
    ##max_page_number = so.find(attrs={'data-hook': 'total-review-count'})
    print so(text=re.compile('Showing 1-10 of'))
    max_page_number_str = so(text=re.compile('Showing 1-10 of'))
    logging.info('Length of the list is {}.'.format(len(max_page_number_str) ))
    if len(max_page_number_str) != 0:
        max_page_number = dig_return = re.findall('\d*\.?\d+', max_page_number_str[0])[2]
    else :
        max_page_number = dig_return = None

    print('Max Page Number:')
    logging.info(max_page_number)
    ...
    max_page_number = int(math.ceil(max_page_number))
    logging.info(max_page_number)
