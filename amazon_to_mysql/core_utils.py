...
def get_reviews_filename(product_id):
    ...
    logging.info('get_reviews_filename --------------- product id [{}] .'.format(product_id))
...
def persist_comment_to_disk(reviews):
    ...
    with open(output_filename, mode='w', encoding='utf-8') as fp:
#        json.dump(reviews.encode('utf-8'), fp, sort_keys=True, indent=4, ensure_ascii=False)
#        json.dump(reviews, fp, sort_keys=True, indent=4, ensure_ascii=False)
        fp.write(unicode(json.dumps(reviews,sort_keys=True, indent=4, ensure_ascii=False)))
...
def get_soup(url):
    ...
    file = open("out.txt", "w")
    file.write(out.text.encode('ascii', 'ignore').decode('ascii'))
    file.close()
    #print('CONTENT')
    #print(out.content)
    assert out.status_code == 200
    soup = BeautifulSoup(out.content, "html5lib")
    print soup
